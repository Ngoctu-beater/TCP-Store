...
async function loadAvailableVouchers() {
  try {
    const res = await fetch(
      `${AppConfig.ORDER_API_URL}/vouchers/public/active`,
    );
    if (res.ok) {
...
async function applyVoucher(code) {
  ...
  try {
    const token = AuthUtils.getToken();
    const res = await fetch(
      `${AppConfig.ORDER_API_URL}/vouchers/validate?code=${code}&subTotal=${globalSubTotal}`,
      {
        headers: { Authorization: `Bearer ${token}` },
      },
    );
...
async function loadAddresses() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      console.error(
        "Không thể tải danh sách địa chỉ, HTTP Status:",
        res.status,
      );
      showEmptyAddressUI();
    }
...
function updateAddressUI() {
  ...
  document.getElementById("display-receiver-name").innerText =
  ...
  document.getElementById("order-receiver-name").value =
    selectedAddress.receiverName;
  document.getElementById("order-receiver-phone").value =
    selectedAddress.phoneNumber;
...
function showEmptyAddressUI() {
  document.getElementById("display-receiver-name").innerText =
    "Chưa có địa chỉ";
  document.getElementById("display-receiver-phone").innerText = "";
  document.getElementById("display-full-address").innerHTML =
    `<span class="text-red-500">Vui lòng thêm địa chỉ nhận hàng trước khi thanh toán!</span>`;
}
...
function confirmAddressSelection() {
  const checkedRadio = document.querySelector(
    'input[name="modal-address"]:checked',
  );
  if (checkedRadio) {
...
async function placeOrder() {
  ...
  // 1. LẤY THÔNG TIN THANH TOÁN & ĐỊA CHỈ
  ...
  if (!receiverName || !shippingAddress) {
    alert("Vui lòng điền thông tin nhận hàng!");
    return;
  }

  // Lấy các sản phẩm đang được tích chọn
  ...
  try {
    ...
    if (response.ok) {
      ...
      // CHỈNH SỬA: Không xóa sản phẩm khỏi giỏ hàng ở đây nữa.
      // Việc xóa giỏ hàng sẽ được xử lý tại trang payment_success.html

      // 2. ĐIỀU HƯỚNG THEO PHƯƠNG THỨC THANH TOÁN
      if (selectedPaymentMethod === "BANK_TRANSFER") {
        const payosRes = await fetch(
          `${AppConfig.ORDER_API_URL}/orders/${orderCode}/generate-payos-link`,
          {
            method: "POST",
            headers: { Authorization: `Bearer ${token}` },
          },
        ...
        if (payosRes.ok) {
          const checkoutUrl = await payosRes.text();
          window.location.href = checkoutUrl; // Chuyển sang trang quét mã PayOS
        } else {
          alert("Không thể tạo link thanh toán PayOS. Vui lòng thử lại!");
        }
      } else {
        // Đối với COD, chuyển thẳng đến trang thành công
        window.location.href = `payment_success.html?orderCode=${orderCode}`;
      }
    } else {
    ...
    console.error(error);
    alert("Lỗi kết nối hệ thống khi đặt hàng!");
  } finally {
    btn.disabled = false;
  }
}