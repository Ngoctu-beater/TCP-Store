...
// Biến toàn cục lưu trữ và xử lý tab
let allUserOrders = [];
...
async function loadMyOrders() {
    ...
    try {
        ...
        // Lưu trữ lại vào biến toàn cục để filter qua lại giữa các Tab
        allUserOrders = orders || [];
        myOrdersData = allUserOrders;

        // Hiển thị ra màn hình theo Tab đang chọn
        renderOrders();
...
function changeOrderTab(status, btnElement) {
    ...
    allTabs.forEach(tab => {
        tab.classList.remove('text-primary', 'border-primary', 'font-bold');
        tab.classList.add('text-gray-500', 'border-transparent', 'font-medium');
    });

    // Bật màu xanh và làm đậm chữ cho DUY NHẤT cái tab vừa được bấm
...
function renderOrders() {
    ...
    const filteredOrders = currentTabStatus === 'ALL' 
        ? allUserOrders 
    ...
    if (filteredOrders.length === 0) {
        container.innerHTML = `
            <div class="bg-white dark:bg-[#1e2125] p-12 text-center rounded-xl border border-gray-100 dark:border-gray-800 flex flex-col items-center">
                ...
                <a href="products.html" class="px-6 py-3 bg-primary text-[#101818] font-bold rounded-xl hover:bg-primary/90 transition-colors">Mua sắm ngay</a>
            </div>
        `;
        return;
    }
    ...
    container.innerHTML = filteredOrders.map((order) => {
        ...
        const itemsHtml = order.items.map((item) => `
            <div class="p-4 md:p-6 border-b border-gray-50 dark:border-gray-800/50 hover:bg-gray-50/50 dark:hover:bg-[#25282c]/50 transition-colors cursor-pointer" onclick="window.location.href='product_detail.html?id=${item.productId}'">
...
async function cancelOrder(orderCode) {
    ...
    try {
        const token = AuthUtils.getToken();
        const response = await fetch(`${AppConfig.ORDER_API_URL}/orders/${orderCode}/cancel`, {
            method: "PUT",
            headers: { Authorization: `Bearer ${token}` },
        });
...
async function buyAgain(orderIdentifier, btnElement) {
    const order = allUserOrders.find((o) => (o.orderCode || o.id).toString() === orderIdentifier.toString());
...
function setRating(stars) {
    ...
    starElements.forEach((el, index) => {
        if (index < stars) {
            el.className = "material-symbols-outlined cursor-pointer text-yellow-400 filled text-4xl transition-transform hover:scale-110";
        } else {
            el.className = "material-symbols-outlined cursor-pointer text-gray-300 text-4xl transition-transform hover:scale-110";
        }
    });
}
...
async function submitReview(btnElement) {
    ...
    try {
        const token = AuthUtils.getToken();
        const response = await fetch(`${AppConfig.PRODUCT_API_URL}/products/${currentReviewProductId}/reviews`, {
            method: "POST",
            headers: {
                Authorization: `Bearer ${token}`,
                "Content-Type": "application/json",
